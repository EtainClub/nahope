...
export default function CannesCountdown() {
  ...
  // Countdown timer simulation ( الفرنسية Cannes closing countdown )
  useEffect(() => {
    const timer = setInterval(() => {
      setTimeLeft((prev) => {
        if (prev.seconds > 0) {
          ...
          // Restart simulation if complete
  ...
  return (
    <div className="w-full max-w-7xl mx-auto grid grid-cols-1 lg:grid-cols-2 gap-6 items-stretch mb-8 px-4 md:px-0">
      ...
      <div className="glass-panel rounded-2xl p-6 border border-neon-pink/20 relative overflow-hidden flex flex-col justify-between shadow-[0_0_20px_rgba(255,0,127,0.05)]">
        ...
        <div className="absolute bottom-0 left-0 w-2 h-2 border-b-2 border-l-2 border-neon-pink rounded-bl" />
        ...
        <div className="grid grid-cols-3 gap-4 py-6 text-center">
          <div className="bg-space-950/80 rounded-xl p-3 border border-space-800">
            <div className="font-righteous text-4xl sm:text-5xl text-white font-bold font-mono tracking-tight text-neon-pink">
              {String(timeLeft.hours).padStart(2, "0")}
            </div>
            <div className="text-[10px] text-gray-500 font-mono font-bold mt-1 uppercase">HOURS</div>
          </div>
          <div className="bg-space-950/80 rounded-xl p-3 border border-space-800">
            <div className="font-righteous text-4xl sm:text-5xl text-white font-bold font-mono tracking-tight">
        ...
        <div className="text-[10px] text-neon-pink font-mono text-center tracking-wider">
          STATUS: TRANSMITTING SYSTEM SIGNALS TO LA CROISETTE
        </div>
      </div>

      {/* Pump.fun Graduation progress */}
      <div className="glass-panel rounded-2xl p-6 border border-neon-purple/20 relative overflow-hidden flex flex-col justify-between shadow-[0_0_20px_rgba(216,0,255,0.05)]">
